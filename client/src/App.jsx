...
function buildGeminiPrompt(prices, market, commodity, dates) {
  return `You are MarketScan AI. Data: prices for ${commodity} in ${market} for dates ${dates.join(
    ", "
  )}: [${prices.join(", ")}]. Latest: ${
    prices[prices.length - 1]
  }. Task: Provide a 1-sentence 1-day price prediction, a confidence score (40-95%) based on 7 days data, and a short actionable tip (10 words max). Format as: Prediction: ... / Confidence: ... / Tip: ...`;
}
...
async function fetchGeminiPrediction(prices, market, commodity, dates) {
  ...
  const text =
    ...
    "Prediction: Stable / Confidence: 60% / Tip: Monitor multiple vendors.";
  return { text };
}
...
function App() {
  ...
  async function handleCheckPrice() {
    ...
    try {
      ...
      const { text } = await fetchGeminiPrediction(
        prices,
        selectedMarket,
        selectedCommodity,
        dates
    ...
    <div style={{ 
      ...
      <div style={{ 
            ...
            <div>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ color: "#6b7280", fontSize: "0.8rem", fontWeight: 500, marginBottom: "4px" }}>
                    Market
                  </div>
                ...
                <div style={{
                  ...
                  <div style={{ 
                    fontWeight: 600, 
                    fontSize: "1.1rem",
                    color: isPositiveChange ? "#059669" : isNegativeChange ? "#dc2626" : "#6b7280",
                  }}>
                    {changePct !== null ? (
                      <span>
                        {isPositiveChange ? "↗" : isNegativeChange ? "↘" : "→"} {changePct.toFixed(1)}%
                      </span>
                    ) : "—"}
                  </div>
                </div>
              ...
              <div>
                ...
                <div style={{
                  ...
                  {prediction || (loading ? "🤔 Generating prediction..." : "Select market and commodity, then click Check Price")}
                </div>
              </div>
            </div>
          </div>
        ...
        <div style={{ 
          ...
          💡 <strong>Pro tip:</strong> Set VITE_GEMINI_API_KEY in a .env file to enable live AI predictions
        </div>
      </div>
    </div>
  );
